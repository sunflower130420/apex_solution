...
void Overlay::RenderMenu()
{
	...
	ImGui::SetNextWindowSize(ImVec2(490, 225));
	ImGui::Begin(XorStr("sunflower"), (bool*)true, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysVerticalScrollbar);
	if (ImGui::BeginTabBar(XorStr("Tab")))
	{
		if (ImGui::BeginTabItem(XorStr("Main")))
		{
			...
			ImGui::Checkbox(XorStr("Triggerbot"), &trigger);
			ImGui::SameLine();
			ImGui::Checkbox(XorStr("NEWTriggerbot"), &strigger);
		...
		if (ImGui::BeginTabItem(XorStr("Config")))
		{
			...
			ImGui::SliderFloat(XorStr("##1"), &espDist, 100.0f, 15000.0f, "%.2f");
			ImGui::SameLine();
			ImGui::Text("(%d meters)", (int)(espDist));

			ImGui::Text(XorStr("Smooth aim value:"));
			ImGui::SliderFloat(XorStr("##2"), &smooth, 12.0f, 350.0f, "%.2f");

			ImGui::Text(XorStr("Max FOV:"));
			ImGui::SameLine(0, 70.0f);
			ImGui::Checkbox(XorStr("FOV"), &v.FOV);
			ImGui::SliderFloat(XorStr("##3"), &max_fov, 1.0f, 25.0f, "%.2f");

			ImGui::Text(XorStr("Aim at (bone id):"));
			ImGui::SliderInt(XorStr("##4"), &bone, 0, 8);
			ImGui::SameLine();
			ImGui::Checkbox(XorStr("Best bone aim"), &bestbone_bool);
		...
		if (ImGui::BeginTabItem(XorStr("Visuals")))
		{
			...
			ImGui::Checkbox(XorStr("seer esp"), &v.seerbar);
			ImGui::Checkbox(XorStr("Entcount"), &v.simplespec);
			ImGui::Checkbox(XorStr("Spectator list"), &v.spectator);
		...
		{
			ImGui::Checkbox(XorStr("Skin changer"), &skinEnable);
			if (skinEnable)
			{
				ImGui::Text(XorStr("WeaponSkinId:"));
				ImGui::SliderInt(XorStr("##1"), &wp_skin_id, 0, 30);
				ImGui::Text(XorStr("SkinId:"));
				ImGui::SliderInt(XorStr("##2"), &skin_id, 0, 30);
			}
			ImGui::EndTabItem();
		}
		ImGui::EndTabBar();
...
DWORD Overlay::CreateOverlay()
{
	...
	io.Fonts->AddFontFromFileTTF("C:\\Users\\Sunflower\\Documents\\apex_guest\\Client\\Helvetica.ttf", 14.0f);
	io.Fonts->Build();*/
...
void Overlay::DrawCornerBox(ImVec2 rect, ImVec2 size, ImColor color, float width)
{
	size.x - 5;
...
void Overlay::DrawCircleFOV(ImVec2 center, float radius, ImColor col, float thick)
{
...
void Overlay::DrawQuadFilled(ImVec2 p1, ImVec2 p2, ImVec2 p3, ImVec2 p4, ImColor color) {
	ImGui::GetWindowDrawList()->AddQuadFilled(p1, p2, p3, p4, color);
}
void Overlay::DrawHexagon(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, const ImVec2& p5, const ImVec2& p6, ImU32 col, float thickness)
{
	ImGui::GetWindowDrawList()->AddHexagon(p1, p2, p3, p4, p5, p6, col, thickness);
}
void Overlay::DrawHexagonFilled(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, const ImVec2& p5, const ImVec2& p6, ImU32 col)
{
	ImGui::GetWindowDrawList()->AddHexagonFilled(p1, p2, p3, p4, p5, p6, col);
}

void Overlay::DrawSeerLikeHealth(float x, float y, int shield, int max_shield, int armorType, int health, float size) {
...
void Overlay::DrawFilledArrow(float x, float y, float size, ImColor color, float yaw)
{
...
world Olympus(ImVec2(-27204.990234, -6551.791992), ImVec2(699, 614), ImVec2(8493.144531, 27624.617188), ImVec2(1101, 229)); //to be measured//1920x1080
world StormPoint(ImVec2(5.449219, 114.082039), ImVec2(662, 831), ImVec2(14.589845, 180.351562), ImVec2(1842, 343));//2560x1440
world BrokenMoon(ImVec2(-33675.039062, -2687.520320), ImVec2(738, 728), ImVec2(35736.964844, 29395.080078), ImVec2(1831, 222));//2560x1440
//Arena
...
void Overlay::drawPlayerOnFullRadar(float x, float y, ImColor color, float yaw, int teamId) {
