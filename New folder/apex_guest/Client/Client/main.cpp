			...
			for (int i = 0; i < 100; i++)
			{
				...
				if (players[i].health > 0)
				{
					...
					else if (dist < espDist)
					{
						...
						{
							/*std::string name_id;
							name_id = XorStr("id:") + std::to_string(players[i].entity_team)+ XorStr("-") + players[i].name;*/
						...
						if (v.FOV)
						{
							DrawCircleFOV(ImVec2(1920 / 2, 1080 / 2), max_fov * 8, WHITE, 1.0f);
						}
					}
				}
			}
...
int main(int argc, char** argv)
{
	...
	add[30] = (uintptr_t)&bestbone_bool;
	add[31] = (uintptr_t)&mapradartest;
	...
	while (active)
	{
		...
		//press f2 for rage mode, turn smooth to 10, fov to 12
		/*if (IsKeyDown(VK_F2)&& k_f2 ==0)
		{
			rage_mode = !rage_mode;
			k_f2 = 1;
			
		}
		else if (!IsKeyDown(VK_F2) && k_f2 == 1)
		{
			k_f2 = 0;
